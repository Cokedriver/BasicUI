...
if cfg.chat.enable ~= true then return end
...
if (cfg.chat.enableBorderColoring) then
...
hooksecurefunc('FloatingChatFrame_OnMouseScroll', function(self, direction)
    ...
    if (cfg.chat.enableBottomButton) then
...
function SkinTab(self)
    ...
    tabText:SetWidth(60)
    if (cfg.chat.tab.fontOutline) then
        tabText:SetFont('Fonts\\ARIALN.ttf', cfg.chat.tab.fontSize, 'THINOUTLINE')
        tabText:SetShadowOffset(0, 0)
    else
        tabText:SetFont('Fonts\\ARIALN.ttf', cfg.chat.tab.fontSize)
    ...
    local s1, s2, s3 = unpack(cfg.chat.tab.specialColor)
    local e1, e2, e3 = unpack(cfg.chat.tab.selectedColor)
    local n1, n2, n3 = unpack(cfg.chat.tab.normalColor)
