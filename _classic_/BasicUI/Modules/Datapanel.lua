...
local defaults = {
	profile = {
		...
		-- Stat Locations
		bags			= "P6",		-- show space used in bags on panel.	
		system 			= "P4",		-- show total memory and others systems info (FPS/MS) on panel.	
		guild 			= "P3",		-- show number on guildmate connected on panel.
		dur 			= "P5",		-- show your equipment durability on panel.
		friends 		= "P3",		-- show number of friends connected.
		spec 			= "P0",		-- show your current spec on panel.
		coords 			= "P0",		-- show your current coords on panel.
		pro 			= "P0",		-- shows your professions and tradeskills
		stats 			= "P0",		-- Stat Based on your Role (Avoidance-Tank, AP-Melee, SP/HP-Caster)
...
function MODULE:CreatePanels()
	...
	-----------------------------------------------------------------------
	DataP2:SetPoint("BOTTOMLEFT", UIParent, "BOTTOMLEFT", 15, 5)
	DataP2:SetWidth(1000 / 3)
	DataP2:SetBackdrop({ 
		bgFile = [[Interface\DialogFrame\UI-DialogBox-Background-Dark]], 
	})
...
function MODULE:CreateStats()
			...
			for i = 0,NUM_BAG_SLOTS do
				...
				if i > 0 then
					...
					if itemLink then
						...
						if (subtype == 'Quiver') or (subtype == 'Ammo Pouch') or (subtype == 'Soul Bag') or (subtype == 'Mining Bag') or (subtype == 'Gem Bag') or (subtype == 'Engineering Bag') or (subtype == 'Enchanting Bag') or (subtype == 'Herb Bag') or (subtype == 'Inscription Bag') or (subtype == 'Leatherworking Bag') or (subtype == 'Fishing Bag')then
							isBag = false
						end
					end
				end
	...
	if db.friends then
		...
		local function BuildFriendTable(total)
			...
			for i = 1, total do
				name, level, class, area, connected, status, note = C_FriendList_GetFriendInfo(i)
		...
		friendsPlugin:RegisterEvent("CHAT_MSG_SYSTEM")
	...
	if db.guild then
		...
		local groupedTable = { "|cffaaaaaa*|r", "" } 
		local MOBILE_BUSY_ICON = "|T\Interface\\ChatFrame\\UI-ChatIcon-ArmoryChat-BusyMobile:14:14:0:0:16:16:0:16:0:16|t";
		local MOBILE_AWAY_ICON = "|T\Interface\\ChatFrame\\UI-ChatIcon-ArmoryChat-AwayMobile:14:14:0:0:16:16:0:16:0:16|t";
	...
	if db.pro then
		...
		local function Update(self)
			Text:SetFormattedText(hexa.."Skills"..hexb)
			self:SetAllPoints(Text)
		end
		...
		proPlugin:SetScript('OnEnter', function()		
			...
			GameTooltip:SetOwner(panel, anchor, xoff, yoff)
			GameTooltip:ClearLines()
			GameTooltip:AddLine(hexa..PLAYER_NAME.."'s"..hexb.." Skills")
			GameTooltip:AddLine' '		
	...
	if db.system then
		...
		local function Update(self, t)
			...
			if int2 < 0 then
				...
				local displayFormat = string.join("", hexa.."FPS: "..hexb, statusColors[fpscolor], "%d|r")
