...
function Nameplates:OnEnable()
	...
	local function UpdateThreatColor(self)
		...
		if (lowThreat and not self.Glow:IsVisible() and isEnemy and db.enableTankMode) then
			...
			if (db.colorNameWithThreat) then
	...
	local function UpdateHealthText(self)
		...
		if (perc >= 100 and currentValue > 5 and db.showFullHP) then
	...
	options = {
		...
		args = {
			...
			enable = {
				type = "toggle",
				order = 1,
				name = L["Enable Nameplates Module"],
				width = "full",
			},
			enableTankMode = {
				type = "toggle",
