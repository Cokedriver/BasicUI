...
if C['general'].nameplates.enable ~= true then return end

--[[

	All Credit for Nameplates.lua goes to Neal and ballagarba.
...
local borderColor = {B.ccolor, B.ccolor, B.ccolor}
local noThreatColor = {0, 1, 0}

local nameplateFlashTexture = 'Interface\\TargetingFrame\\UI-TargetingFrame-Flash'

local glowTexture = 'Interface\\AddOns\\BasicUI\\Media\\NameplateGlow'
...
local function IsTarget(self) 
    ...
    if (targetName == self.Name:GetText() and self:GetAlpha() >= 1) then
        return true
    else
        return false
    end
...
local function UpdateThreatColor(self)
    ...
    if (lowThreat and not self.Glow:IsVisible() and isEnemy and C['general'].nameplates.enableTankMode) then
        ...
        if (C['general'].nameplates.colorNameWithThreat) then
...
local function UpdateHealthText(self)
    ...
    if (perc >= 100 and currentValue > 5 and C['general'].nameplates.showFullHP) then
...
local function UpdateNameL(self)
    ...
    if (C['general'].nameplates.abbrevLongNames) then
        newName = (len(newName) > 20) and gsub(newName, '%s?(.[\128-\191]*)%S+%s', '%1. ') or newName
    end
    
    self.NewName:SetTextColor(1, 1, 1)
    if (C['general'].nameplates.showLevel) then
        ...
        elseif (eliteTexture) then
            self.NewName:SetText(levelColor..levelText..'|r '..'|cffffff00Elite |r'..newName)			
        else
...
local function UpdatePlate(self)
    ...
    if (C['general'].nameplates.showEliteBorder) then
    ...
    if (C['general'].nameplates.showTotemIcon) then
...
local function SkinPlate(self)
    ...
    self.Overlay:SetPoint('TOPRIGHT', self.Health, 30, 4)
    self.Overlay:SetPoint('BOTTOMLEFT', self.Health, -30, -4)
    ...
    if (not self.Health.Value) then    
        ...
        self.Health.Value:SetFont('Fonts\\ARIALN.ttf', 10)
        --self.Health.Value:SetShadowOffset(0, 0)
    end

    if (not self.NewName) then
        self.NewName = self:CreateFontString(nil, 'ARTWORK')
        self.NewName:SetFont('Fonts\\ARIALN.ttf', 12, 'THINOUTLINE')
        self.NewName:SetShadowOffset(0, 0)
        self.NewName:SetPoint('BOTTOM', self.Health, 'TOP', 0, 4)
        self.NewName:SetSize(105, 13)
    end
    ...
    self.Castbar.Overlay:SetPoint('TOPRIGHT', self.Castbar, 30, 4)
    self.Castbar.Overlay:SetPoint('BOTTOMLEFT', self.Castbar, -30, -4)
    ...
    f:HookScript('OnUpdate', function(_, elapsed)
        ...
        if (f.elapsed >= 0.1) then
            ...
            if (C['general'].nameplates.showTargetBorder) then
