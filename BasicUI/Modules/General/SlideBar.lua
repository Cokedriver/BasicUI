...
if lib.frame then
	...
	frame:SetScript("OnEvent", function(self, event, arg, ...)
		...
		elseif event == "ADDON_LOADED" and arg == "SlideBar" then
			...
			if not BasicDB or type(BasicDB) == "string" then
				BasicDB = {}
			end
...
function private:BeginMove(...)
	if BasicDB.locked == "1" then return end
	local button = ...
...
function private.CommandHandler(msg)
	...
	if (tonumber(a)) then
		BasicDB.position = math.min(math.abs(tonumber(a)), 1200)
		save = true
	end
	if (a == "fadeout" or a == "fade") then
		BasicDB.visibility = "1"
		save = true
	elseif (a == "nofade") then
		BasicDB.visibility = "0"
		save = true
	end
	if (a == "size") then
		if (tonumber(b)) then
...
function private.boxMover()
	...
	local anchor = BasicDB.anchor or "right"
		...
		for pos = 1, #GUI do
			...
			if button then
				if  GUI[pos] and button.icon then
					...
					if BasicDB[button.id..".hide"] then
